...
* `fleet-controller` logs on the management cluster: has {product_name} failed to reconcile any resource's (bundle, bundledeployment) current state with its expected state?
...
If the issue is specific to a target cluster, one might want to check Fleet agent logs on that cluster: has {product_name} failed.
...
==== Via {product_name} Install Options
...
Modifying the config will re-install {product_name} and re-deploy the agents.
...
The cluster will show in `WaitCheckIn` status because the `fleet-controller` is attempting to communicate with {product_name} using the Rancher service IP. However, {product_name} must communicate directly with Rancher via the Kubernetes service DNS using service discovery, not through the proxy.
