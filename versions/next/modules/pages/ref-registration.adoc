...
{product_name} passes these values to a Fleet agent, so it can connect back to the Fleet controller.
...
{product_name} uses this kubeconfig to deploy the agent on the downstream cluster.
